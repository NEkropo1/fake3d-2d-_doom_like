...
class NPC(AnimatedSprite):
    def __init__(
        ...
        self.player_dist_v = 0
        self.player_dist_h = 0
    ...
    def attack(self):
        if self.animation_trigger:
            if type(self) == CacoDemonNPC:
                self.game.sound.npc_caco_attack.play()
            else:
    ...
    def check_hit_in_npc(self):
        if self.ray_cast_value and self.game.player.shot:
            if (
                ...
                if type(self) == CacoDemonNPC:
                    self.game.sound.npc_caco_pain.play()
                else:
    ...
    def check_health(self):
        if self.health < 1:
            self.alive = False
            if type(self) == CacoDemonNPC:
                self.game.sound.npc_caco_death.play()
            else:
    ...
    def ray_cast_player_npc(self):
        ...
        player_dist = max(self.player_dist_v, self.player_dist_h)
